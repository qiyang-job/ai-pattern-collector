...
把 Pattern 放入固定研究框架。五个核心分类字段职责严格区分：

- **Product Category** — 产品形态：这是什么形态的 AI 产品（与单张截图无关）
- **Journey Stage** — 用户路径：任务流程中的哪个步骤（宏观纵向阶段）
- **Screenshot State** — 界面状态：截图那一刻 UI 处于什么操作态（微观瞬时）
  - 配套 `secondaryScreenshotStates`（次要状态，可多选/留空）与 `screenshotStateReason`（判定理由）
- **Pattern Category** — 模式分类：提炼出的模式解决哪一类设计问题
- **Reuse Level** — 复用价值：该模式是否值得复用到其他 AI 产品

此外还有 Lens Score（8 维评分）。这一层决定记录会进入 Matrix、Journey、Library、Insights。

UI 中要让用户理解：分类不是为了填字段，而是为了让记录能被比较、归纳和复用。Matrix 主轴固定为 Product Category × Journey Stage，Screenshot State 不进入主轴。
...
回答「这是什么形态的 AI 产品」，描述截图所属产品的整体交互范式（与单张截图的界面状态无关）。

- AI Chat — 以多轮对话为主交互的通用助手
- AI Search — 以检索 + 答案生成为核心的产品
- AI Agent — 可自主规划、调用工具并执行多步任务的智能体（原 `Agent Task`，已迁移）
- AI Workspace — 围绕文档/画布/表格协作的 AI 工作台
- Coding Agent — 面向代码库的编程智能体

> 迁移：历史值 `Agent Task` 自动归一为 `AI Agent`。
...
回答「截图那一刻 UI 处于什么操作态」，是微观瞬时的界面状态（区别于宏观的 Journey Stage）。共 14 项（含 Unknown 兜底）：

Idle · Inputting · Context Ready · Thinking · Planning Ready · Running · Waiting Approval · Streaming · Reviewing · Error · Completed · Follow-up Ready · Export Ready · Unknown

> 迁移：旧「路径/组件」导向值（Default / Input Assist / Context Attach / Plan Preview / Tool Call / Human Approval / Partial Result / Error Recovery / Final Result / Follow-up / Export / Handoff）已自动归一为对应新界面状态值（如 Default → Idle、Tool Call → Running、Final Result → Completed）。
>
> 一张截图可能同时呈现多个状态：用 `screenshotState` 记录主状态，`secondaryScreenshotStates` 记录次要状态（可为空），`screenshotStateReason` 记录判定理由。

### 4.4 Pattern Category

回答「提炼出的模式解决哪一类设计问题」。固定顺序，共 8 类：
...
- Refinement Patterns — 优化迭代模式（新增）
- Output Handoff Patterns — 输出交接模式
- Failure Recovery Patterns — 失败恢复模式（新增）
...
- Library 页面即使没有数据，也要展示 8 类 Pattern Category
...
Library 是模式库。必须始终展示 8 类 Pattern Category（含新增的 Refinement Patterns 与 Failure Recovery Patterns），空分组也要展示。每条 Pattern 必须能追溯 Evidence Screenshot。
