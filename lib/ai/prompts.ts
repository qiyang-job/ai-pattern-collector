...
- 必须是中文短语，3-8 个字，描述一种可复用的交互模式。
- 不要用产品名，不要太泛，不要写成页面名称。
- 正确示例：可编辑计划预览 / 人工审批门 / 来源引用式回答 / 工具调用时间线 / 上下文附加标签 / 文件变更差异预览 / 渐进式结果流 / 可恢复错误状态。
- 错误示例：Cursor 规划 / AI 结果 / 聊天界面 / 搜索页面 / 用户输入 / 分析功能。
...
- AI Agent：多步骤任务、调用浏览器/工具/外部系统、计划-执行-等待-审批（ChatGPT Agent/Manus/Operator 类/browser-use 类）。注：旧名 Agent Task 已更名为 AI Agent。
...
# Screenshot State 判断规则（关键：是界面此刻的操作态，不是路径阶段）
⚠️ Journey Stage 问"用户在流程哪一步"，Screenshot State 问"界面此刻是什么操作态"。两者维度不同。
- Idle：空闲/空白初始态/首页未开始/光标在输入框但无内容。
- Inputting：用户正在输入或编辑指令/输入框有内容/附件选择中。
- Context Ready：上下文已附加就绪但未开始处理/文件已上传/Context chip 已展示。
- Thinking：AI 已接收请求、正在思考但无可见输出/thinking/转圈/reasoning 折叠。
- Planning Ready：计划/步骤/待办已生成等待查看或确认/plan 列表/TODO/diff 预览。
- Running：正在执行工具调用或多步任务/terminal 运行/tool call/进度条/step 推进。
- Waiting Approval：执行暂停等待批准或授权/确认弹窗/Allow/Run/权限请求。
- Streaming：答案正在逐字流式输出/光标闪烁/文本增量/Stop 按钮。
- Reviewing：结果已产出、正在审阅校验/diff 审查/引用核对/对比视图。
- Error：出错/超时/被阻断/报错信息/失败态/重试按钮。
- Completed：任务已完成、结果稳定/完整答案/成功态/无进行中指示。
- Follow-up Ready：结果已给出并引导继续追问或下一步/追问建议/相关问题。
- Export Ready：结果可复制/导出/交接/导出菜单/分享/下载/复制代码块。
- Unknown：无法从截图明确判断界面状态时使用（仅作兜底，应尽量避免）。

# 多界面状态规则（重要）
一张截图可能同时呈现多个界面状态：
- screenshotState：选「最主要/最能代表当前焦点」的单个状态。
- secondaryScreenshotStates：同一截图中同时存在但非主导的其他状态数组（可为空 []）；不要重复主状态，不要放 Unknown。
- screenshotStateReason：用一句中文写出判定主状态（及次要状态）的可见证据依据，不允许留空。

# Pattern Category 判断规则（共 8 类，按顺序）
...
- Refinement Patterns：用户基于结果继续追问/修改/重试/细化迭代（Follow-up、Regenerate、Edit、Branch、Refine、Continue、Try again、Version、Variant）。
- Output Handoff Patterns：帮助结果进入下一步工作流（Export、Copy、Save、Share、Apply、Commit、Publish、Create Artifact）。
- Failure Recovery Patterns：出错/超时/能力边界时如何解释/兜底/恢复（Error Explanation、Retry、Fallback、Undo、Graceful Degradation、Capability Limit Notice、Safe Stop）。
...
# 语言规则
分析内容统一用中文。例外：productCategory/journeyStage/screenshotState/patternCategory/reuseLevel 用固定英文枚举；tags 可用英文短标签。
不要输出 Markdown，不要输出 JSON 之外的任何解释性文字。

# 输出 JSON Schema（严格输出，不要包裹代码块）
{
  "product": "string",
  "productCategory": "AI Chat | AI Search | AI Agent | AI Workspace | Coding Agent",
  ...
  "screenshotState": "Idle | Inputting | Context Ready | Thinking | Planning Ready | Running | Waiting Approval | Streaming | Reviewing | Error | Completed | Follow-up Ready | Export Ready | Unknown",
  "secondaryScreenshotStates": ["同上枚举的次要界面状态，可为空数组"],
  "screenshotStateReason": "判定界面状态的中文依据，不可留空",
  "patternName": "string",
  "patternCategory": "Intent Input Patterns | Context Management Patterns | Planning & Reasoning Patterns | Execution Feedback Patterns | Trust & Verification Patterns | Refinement Patterns | Output Handoff Patterns | Failure Recovery Patterns",
  "userProblem": "string",
  "aiCapability": "string",
  "uiAnatomy": "string",
...
export function buildAnalyzeUserText(input: {
  ...
  return [
    ...
    "6. screenshotState 选主状态；secondaryScreenshotStates 列同时存在的次要状态(可空)；screenshotStateReason 写出判定依据，禁止留空。",
    "7. 输出严格 JSON，不要 Markdown，不要解释性文字。",
  ].join("\n");
}
...
# 分析目标
1. Research Scope：判断当前样本覆盖了哪些产品、产品类型、用户流程阶段、界面状态和模式分类。
...
6. High-value Patterns：识别高复用价值 Pattern，说明它们为什么值得复用。
7. Cross-product Comparison：比较不同产品类型在 AI 体验设计上的差异。
8. Stage Maturity：判断不同用户阶段的设计成熟度，尤其关注 J-03 到 J-07。
9. Missing States：指出尚未被采集到的界面状态/分类盲区，给出补采方向。
10. Design Opportunities：指出当前样本中暴露出的设计机会点。
