...
function buildUserText({ product, rawNote, taskContext, sourceUrl, imageCount = 1 }) {
  const imageLabel = imageCount > 1
    ? `已附加 ${imageCount} 张截图（同一流程的不同步骤），请综合分析所有截图后输出一个统一的 Pattern Record。`
    : "已附加截图。";
  return [
    `请分析下面的 ${imageCount > 1 ? '一组' : ''}AI 产品截图，并输出结构化 Pattern Record。`,
    "输入信息：",
    `【Product】\n${product || "用户未填写"}`,
    ...
    ...(imageCount > 1 ? [
      "9. 多图分析提示：这些截图展示了同一交互流程的不同阶段/状态，请识别它们之间的时序关系和完整交互模式。",
    ] : []),
  ].join("\n");
}
...
exports.main = async (event) => {
  try {
    ...
    if (!imageDataUrls.length) return { error: "请先上传或粘贴截图" };
    if (imageDataUrls.length > 5) return { error: "最多支持 5 张截图" };
    ...
    console.log(`[ai-analyze] payload: ${(payloadSize / 1024).toFixed(0)}KB, images=${imageDataUrls.length}`);

    const contextInfo = { product, rawNote, taskContext, sourceUrl };
    const userContent = imageDataUrls.length > 1
      ? buildMultiImageContent(imageDataUrls, contextInfo)
      : [
          { type: "text", text: buildUserText({ ...contextInfo, imageCount: 1 }) },
