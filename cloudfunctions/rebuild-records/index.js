...
// 旧值迁移表（与前端 lib/constants.ts 保持一致）
...
Product Category = 产品形态；Journey Stage = 用户路径(宏观)；Screenshot State = 界面状态(微观瞬时)；Pattern Category = 模式库分类；Reuse Level = 复用价值。
...
Intent Input 帮表达意图；Context Management 获取展示编辑追溯上下文；Planning & Reasoning 让任务拆解计划推理可见；Execution Feedback 展示执行过程/工具调用/进度/部分结果；Trust & Verification 帮判断可信/正确/可追溯；Refinement 基于结果继续追问/修改/重试/细化迭代；Output Handoff 帮结果进入下一步工作流；Failure Recovery 出错/超时/能力边界时如何解释/兜底/恢复。

# Screenshot State 判断（界面此刻操作态，非路径阶段）
Idle 空闲/空白初始态；Inputting 正在输入/输入框有内容；Context Ready 上下文已就绪/附件已添加；Thinking AI 思考处理中/无输出有活动；Planning Ready 计划/步骤/待办已生成；Running 执行中/工具调用/Terminal 输出流；Waiting Approval 等待确认/审批/权限请求；Streaming 流式输出进行中/文字逐渐出现；Reviewing 审阅验证/Diff/对比视图；Error 错误阻断/报错/重试；Completed 任务已完成/完整结果；Follow-up Ready 可继续追问/相关建议；Export Ready 可导出/分享/复制/Commit；Unknown 无法判断(仅兜底)。
多状态规则：screenshotState 选主状态；secondaryScreenshotStates 列同时存在的次要状态(可空数组，不放主状态与 Unknown)；screenshotStateReason 写出判定依据，禁止留空。
...
const USER_TEXT = [
  ...
  "8. ⚠️ 每个字段都必须填写实际内容，禁止空字符串！无法判断时写「截图中未体现」。",
].join("\n");
  ...
  if (event && event.action === "recover") {
    ...
    try {
      ...
      const record = {
        ...
        secondaryScreenshotStates: result.secondaryScreenshotStates,
        screenshotStateReason: result.screenshotStateReason,
